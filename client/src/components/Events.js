...
import { fetchEvents, fetchFilteredEvents } from '../features/events/eventsSlice'; // Импорт fetchEvents и fetchFilteredEvents
...
function Events() {
  ...
  useEffect(() => {
    if (token) {
      dispatch(fetchEvents()); // Замена fetchUserEvents на fetchEvents
  ...
  const handleFilterEvents = () => {
    dispatch(fetchFilteredEvents(filter)); // Использование fetchFilteredEvents для фильтрации событий
  };
